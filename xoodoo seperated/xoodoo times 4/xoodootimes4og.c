...
#if (VERBOSE > 0)
#define DumpOne(__b, __v, __i) \
    STORE128(__b, __v##__i);   \
...
ALIGN(32)
static const uint32_t oAllFrom1_0[] = {1, 2, 3, 4 + 0};
ALIGN(32)
static const uint32_t oAllFrom2_0[] = {2, 3, 4 + 0, 4 + 1};
ALIGN(32)
static const uint32_t oAllFrom3_0[] = {3, 4 + 0, 4 + 1, 4 + 2};

ALIGN(32)
static const uint32_t oLow64[] = {0, 1, 4 + 0, 4 + 1};
ALIGN(32)
static const uint32_t oHigh64[] = {2, 3, 4 + 2, 4 + 3};

ALIGN(32)
static const uint32_t oLow32[] = {0, 4 + 0, 2, 4 + 2};
ALIGN(32)
static const uint32_t oHigh32[] = {1, 4 + 1, 3, 4 + 3};

ALIGN(32)
static const uint32_t oGatherScatterOffsets[] = {0 * 12, 1 * 12, 2 * 12, 3 * 12};
...
#define DeclareVars          \
    V128 a00, a01, a02, a03; \
    V128 a10, a11, a12, a13; \
    V128 a20, a21, a22, a23; \
    V128 v1, v2;
#define State2Vars2                                                     \
    a00 = states[0], a01 = states[1], a02 = states[2], a03 = states[3]; \
...
#define Vars2State                                                      \
    states[0] = a00, states[1] = a01, states[2] = a02, states[3] = a03; \
...
size_t Xooffftimes4_CompressFastLoop(unsigned char *k, unsigned char *x, const unsigned char *input, size_t length)
{
    ...
    x20 = _mm_setzero_si128();printf("%f,%f,%f",RDTSC_clk_min ,RDTSC_clk_median,RDTSC_clk_max );
...
void main()
{
	Xoodootimes4_align512SIMD128_states data;
	Xoodootimes4_align512SIMD128_states *state = &data;
	...
	Xoodootimes4_PermuteAll_12rounds(state);
}
